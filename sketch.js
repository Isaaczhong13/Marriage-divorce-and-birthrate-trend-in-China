...
function setup() {
  ...
  textFont('Poppins');
  setupScene1();
  select('#lineChart').position(0, height * 0.2);
  select('#fullLineChart').position(0, height * 0.2);
...
function setupScene1() {
  select('#lineChart').style('visibility', 'visible');
...
function setupScene2() {
  heads1 = [];
  heads2 = [];
  select('#lineChart').style('visibility', 'hidden');
  select('#fullLineChart').style('visibility', 'hidden');
...
let gRedTexts = ["Unnecessary", "Doesn't matter", "Necessary"];
let sgText = "";
...
function drawScene6() {
  ...
  fill("#f12626");
  text(sgText, textX + width * 0.1, textY + textGap);
